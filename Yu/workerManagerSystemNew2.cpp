...
void InputList()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        cin >> p->data.zhiwei;
        cout << "请输入第" << i + 1 << "个职工的出生年月：" << endl;
        cin >> p->data.birth;
        cout << "请输入第" << i + 1 << "个职工的入职时间：" << endl;
        cin >> p->data.intime;
        cout << "请输入第" << i + 1 << "个职工的学历：" << endl;
        cin >> p->data.study;
        if (head->next == NULL)
        {
            p->next = NULL;
...
void ShowData()
{
    ...
    while (p != NULL)
    {
        cout << p->data.id << "\t" << p->data.name << "\t" << p->data.sex << "\t" << p->data.number << "\t" << p->data.zhiwei << "\t" << p->data.birth << "\t" << p->data.intime << "\t" << p->data.study << "\t" << endl;
        p = p->next;
    }
}
...
void StorageData()
{
    ...
    else
    {
        while (q != NULL)
        {
            fprintf(txt, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", q->data.id, q->data.name, q->data.sex, q->data.number, q->data.qq, q->data.zhiwei, q->data.birth, q->data.intime, q->data.study);
            q = q->next;
        }
    }
    fclose(txt);
    ...
    while (q != NULL)
    {
        if (strcmp(q->data.id, ID) == 0)
        {
            cout << "编号  "
                 ...
                 << "\t"
                 << "学历  " << endl;
            cout << q->data.id << "\t" << q->data.name << "\t" << q->data.sex << "\t" << q->data.number << "\t" << q->data.qq << "\t" << q->data.zhiwei << "\t" << q->data.birth << "\t" << q->data.intime << "\t" << q->data.study << "\t" << endl;
            return;
        }
        q = q->next;
    }
...
void ReviseData()
{
    ...
    cin >> ln->data.zhiwei;
    cout << "请输入修改后的职工出生年月：" << endl;
    cin >> ln->data.birth;
    cout << "请输入修改后的职工入职时间：" << endl;
    cin >> ln->data.intime;
    cout << "请输入修改后的职工学历：" << endl;
    cin >> ln->data.study;
    ...
    strcpy(q->data.birth, ln->data.birth);
    strcpy(q->data.intime, ln->data.intime);
    strcpy(q->data.study, ln->data.study);
    free(ln);
}
...
void InputTxt()
{
    ...
    while (true)
    {
        ...
        if (fscanf(txt, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", p->data.id, p->data.name, p->data.sex, p->data.number, p->data.qq, p->data.zhiwei, p->data.birth, p->data.intime, p->data.study) == EOF) //从txt文件中读入数据
        {
            free(p); //释放p节点空间
            cout << "读入成功！" << endl;
            break;
...
int main()
{
    ...
    while (choice != 0)
    {
        ...
        printf("******************************************************************************************************************************\n");
        printf("|                                                                                                                            |\n");
        printf("|1.添加  2.查找  3.修改  4.删除  5.显示链表数据  6.保存到txt文件  7.显示txt文件数据  8.txt中查找  9.读入txt  10.返回  11.排序|\n");
        printf("|                                                                                                                            |\n");
        printf("******************************************************************************************************************************\n");
        printf("输出需要操作的选项：");
        cout << endl;
        ...
        case 5:
            ...
            break;
        case 6:
            StorageData(); //保存到txt文件
            break;
        case 7:
            DisplayData(); //显示txt文件数据
            break;
        case 8:
            FindInTxt(dataID); // txt中查找
            break;
        case 9:
            InputTxt(); //读入txt
            break;
